        ...
        implements PlanOptimizer
{
    ...
    public PlanOptimizerResult optimize(PlanNode plan, Session session, TypeProvider types, VariableAllocator variableAllocator, PlanNodeIdAllocator idAllocator, WarningCollector warningCollector)
    {
        ...
        PlanNode producerReplaced = SimplePlanRewriter.rewriteWith(new CteProducerRewiter(session, idAllocator, variableAllocator), plan, context);
        PlanNode rewrittenPlan = SimplePlanRewriter.rewriteWith(new CteConsumerRewrite(session, idAllocator, variableAllocator), producerReplaced, context);
            ...
            try {
                ...
                if (e.getErrorCode().equals(NOT_SUPPORTED.toErrorCode())) {
                    throw new PrestoException(
                            NOT_SUPPORTED,
                            format("Temporary table cannot be created in catalog \"%s\": %s", "hive", e.getMessage()),
                            e);
                }
                throw e;
            }
